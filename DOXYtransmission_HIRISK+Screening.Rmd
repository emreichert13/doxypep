...
** Add in the impact of increased screening among 
...
doxy_sim0 %>% mutate(RowSum=rowSums(.[setdiff(names(.),c("time", "Inc", "prevB", "prevA", "prevAB", "prevGC", "ARx", "BRx", "Doxy1", "Doxy2", "Doxy3"))])) %>% 
  select(RowSum, Doxy1, Doxy2) %>% summarise(min(RowSum), max(RowSum), sum(Doxy1), sum(Doxy2))
doxy_sim10 %>% mutate(RowSum=rowSums(.[setdiff(names(.),c("time", "Inc", "prevB", "prevA", "prevAB", "prevGC", "ARx", "BRx", "Doxy1", "Doxy2", "Doxy3"))])) %>% 
  select(RowSum, Doxy1, Doxy2)%>% summarise(min(RowSum), max(RowSum), sum(Doxy1), sum(Doxy2))
doxy_sim25 %>% mutate(RowSum=rowSums(.[setdiff(names(.),c("time", "Inc", "prevB", "prevA", "prevAB", "prevGC", "ARx", "BRx", "Doxy1", "Doxy2", "Doxy3"))])) %>% 
  select(RowSum, Doxy1, Doxy2)%>% summarise(min(RowSum), max(RowSum), sum(Doxy1), sum(Doxy2))
doxy_sim50 %>% mutate(RowSum=rowSums(.[setdiff(names(.),c("time", "Inc", "prevB", "prevA", "prevAB", "prevGC", "ARx", "BRx", "Doxy1", "Doxy2", "Doxy3"))])) %>% 
  select(RowSum, Doxy1, Doxy2)%>% summarise(min(RowSum), max(RowSum), sum(Doxy1), sum(Doxy2))
doxy_sim75 %>% mutate(RowSum=rowSums(.[setdiff(names(.),c("time", "Inc", "prevB", "prevA", "prevAB", "prevGC", "ARx", "BRx", "Doxy1", "Doxy2", "Doxy3"))])) %>% 
  select(RowSum, Doxy1, Doxy2)%>% summarise(min(RowSum), max(RowSum), sum(Doxy1), sum(Doxy2))
doxy_sim90 %>% mutate(RowSum=rowSums(.[setdiff(names(.),c("time", "Inc", "prevB", "prevA", "prevAB", "prevGC", "ARx", "BRx", "Doxy1", "Doxy2", "Doxy3"))])) %>% 
  select(RowSum, Doxy1, Doxy2)%>% summarise(min(RowSum), max(RowSum), sum(Doxy1), sum(Doxy2))
...
```{r}
df_list <- list(doxy_sim0 %>% select(-Inc, -prevB, -prevA, -prevAB, -prevGC, -ARx, -BRx), 
                doxy_sim10 %>% select(-Inc, -prevB, -prevA, -prevAB, -prevGC, -ARx, -BRx), 
                doxy_sim25 %>% select(-Inc, -prevB, -prevA, -prevAB, -prevGC, -ARx, -BRx), 
                doxy_sim50 %>% select(-Inc, -prevB, -prevA, -prevAB, -prevGC, -ARx, -BRx), 
                doxy_sim75 %>% select(-Inc, -prevB, -prevA, -prevAB, -prevGC, -ARx, -BRx),
                doxy_sim90 %>% select(-Inc, -prevB, -prevA, -prevAB, -prevGC, -ARx, -BRx))
...
doxy_sim_all %>%
  ...
  summarise(CumInc5 = CumInc[time == 1825],
            ...
            Time84 = min(time[prevB >= 0.84])/365,
...
#pdf(paste0(filepath, "DOXYPEP_PrevHI.pdf"), height = 6, width = 8)
prev_HI <- ggplot() +
  ...
  theme_classic() + xlab("Years") + ylab("Gonococcal Infection Prevalence (%)") + labs(col = "DoxyPEP Uptake in High\nActivity Group") + theme(text = element_text(size=15), legend.position = c(0.80, 0.25)) +
  scale_color_manual(values = c("#172869", "#6C6C9D","#1BB6AF", "#A6E000", "#FC6882", "#C70E7B"))
prev_HI
#dev.off()
ggsave(file = paste0(filepath, "DOXYPEP_PrevHI+Screening.svg"), plot = prev_HI, height = 6, width = 8)

#pdf(paste0(filepath, "DOXYPEP_IncHI.pdf"), height = 6, width = 8)
inc_HI <- ggplot() +
   ...
   theme_classic() + xlab("Years") + ylab("Incidence Rate (Cases/100 PY)") + labs(col = "DoxyPEP Uptake in High\nActivity Group") + theme(text = element_text(size=15), legend.position = c(0.85, 0.25)) +
   scale_color_manual(values = c("#172869", "#6C6C9D","#1BB6AF", "#A6E000", "#FC6882", "#C70E7B"))
inc_HI
#dev.off()
#ggsave(file = paste0(filepath, "DOXYPEP_IncHI.svg"), plot = inc_HI, height = 6, width = 8)
```
...
doxy_sim_all %>%
  group_by(DoxyPEP) %>%
  summarise(min(prevGC), max(prevGC))
```

```{r}
#Visualize PR and IRR relative to baseline (0% DoxyPEP uptake)
#pdf(paste0(filepath, "DOXYPEP_PRHI.pdf"), height = 3, width = 3)
PR_HI <- ggplot() +
...
PR_HI
#dev.off()
ggsave(file = paste0(filepath, "DOXYPEP_PRHI+Screening.svg"), plot = PR_HI, height = 3, width = 3)

#pdf(paste0(filepath, "DOXYPEP_IRRHI.pdf"), height = 3, width = 3)
IRR_HI <- ggplot() +
  ...
  theme_classic() + xlab("Years") + ylab("Incidence Rate Ratio") + labs(col = "DoxyPEP Uptake") +
   theme(text = element_text(size=15), legend.position = "none") +
   scale_color_manual(values = c("#172869", "#6C6C9D","#1BB6AF", "#A6E000", "#FC6882", "#C70E7B"))
IRR_HI
#dev.off()
#ggsave(file = paste0(filepath, "DOXYPEP_IRRHI.svg"), plot = IRR_HI, height = 3, width = 3)
```
...
infectiondat <- doxy_sim_long_all %>%
  ...
  mutate(ResistLabels = ifelse(ResistState == "Neither", "Neither",
                               ifelse(ResistState == "B only", "Doxycycline\nonly",
                                      ifelse(ResistState == "A only", "Ceftriaxone\nonly",
                                             ifelse(ResistState == "A and B", "Ceftriaxone +\nDoxycycline", NA)))))

#pdf(paste0(filepath, "ResistProfileHI.pdf"), width = 7.5, height = 5)
#plot prevalence of gonorrhea over time, by Resistance Profile
ggplot(infectiondat,aes(x=time/365, y=percent*100, fill = factor(ResistLabels, levels = c("Neither", "Doxycycline\nonly", "Ceftriaxone\nonly", "Ceftriaxone +\nDoxycycline")))) +
