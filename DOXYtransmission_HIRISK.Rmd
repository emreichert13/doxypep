...
#pdf(paste0(filepath, "F1_DOXYPEP_PrevHI.pdf"), height = 6, width = 8)
prev_HI <- ggplot() +
...
prev_HI
#dev.off()
#ggsave(file = paste0(filepath, "F1_DOXYPEP_PrevHI.svg"), plot = prev_HI, height = 6, width = 8)

#pdf(paste0(filepath, "SF1_DOXYPEP_IncHI.pdf"), height = 6, width = 8)
inc_HI <- ggplot() +
...
inc_HI
#dev.off()
#ggsave(file = paste0(filepath, "SF1_DOXYPEP_IncHI.svg"), plot = inc_HI, height = 6, width = 8)
```
...
#pdf(paste0(filepath, "F1_DOXYPEP_PRHI.pdf"), height = 3, width = 3)
PR_HI <- ggplot() +
...
PR_HI
#dev.off()
#ggsave(file = paste0(filepath, "F1_DOXYPEP_PRHI.svg"), plot = PR_HI, height = 3, width = 3)

#pdf(paste0(filepath, "SF1_DOXYPEP_IRRHI.pdf"), height = 3, width = 3)
IRR_HI <- ggplot() +
...
IRR_HI
#dev.off()
#ggsave(file = paste0(filepath, "SF1_DOXYPEP_IRRHI.svg"), plot = IRR_HI, height = 3, width = 3)
```
...
infectiondat <- doxy_sim_long_all %>%
  ...
  mutate(ResistLabels = ifelse(ResistState == "Neither", "Neither",
                               ifelse(ResistState == "B only", "High-level\nTetracycline only",
                                      ifelse(ResistState == "A only", "Ceftriaxone\nonly",
                                             ifelse(ResistState == "A and B", "Dual", NA)))))

infectiondat <- left_join(infectiondat, 
                          resB %>% select(DoxyPEP, MinT),
                          by = "DoxyPEP")

#pdf(paste0(filepath, "F2_ResistProfileHI.pdf"), width = 7.5, height = 5)
#plot prevalence of gonorrhea over time, by Resistance Profile
ggplot(data = infectiondat, aes(x=time/365, y=prev*100, fill = factor(ResistLabels, levels = c("Neither", "High-level\nTetracycline only", "Ceftriaxone\nonly", "Dual")))) +
  geom_area() + 
  scale_fill_manual(values = c("turquoise3", "#E9A17C", "mediumpurple", "deeppink2")) +
  theme_classic() + facet_wrap(~DoxyPEP) +
  xlab("Years") + ylab("Prevalence of Gonococcal Infection (%)") + labs(fill = "Resistance Profile") +
  #theme(legend.position = "top") +
  geom_vline(aes(xintercept = MinT/365), col = "black", linetype = "dashed")
#dev.off()
```
