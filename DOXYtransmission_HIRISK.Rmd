...
DoxyPEP Model Impact
with DoxyPEP only targeted to high sexual activity group (fixed 10% of population)
...
doxyPEP.SI <- function(t, x, parms){
    ...
    list(der, Inc = Inc, prevA = prevA, prevB = prevB, prevAB = prevAB, prevGC = prevGC, ARx = ARx, Doxy = Doxy)
  })
}
```
...
# and that only high risk group is receiving DoxyPEP
doxy_sim0 %>% mutate(RowSum=rowSums(.[setdiff(names(.),c("time", "Inc", "prevB", "prevA", "prevAB", "prevGC", "ARx", "Doxy1", "Doxy2", "Doxy3"))])) %>% 
  select(RowSum, Doxy1, Doxy2) %>% summarise(min(RowSum), max(RowSum), sum(Doxy1), sum(Doxy2))
doxy_sim10 %>% mutate(RowSum=rowSums(.[setdiff(names(.),c("time", "Inc", "prevB", "prevA", "prevAB", "prevGC", "ARx", "Doxy1", "Doxy2", "Doxy3"))])) %>% 
  select(RowSum, Doxy1, Doxy2)%>% summarise(min(RowSum), max(RowSum), sum(Doxy1), sum(Doxy2))
doxy_sim25 %>% mutate(RowSum=rowSums(.[setdiff(names(.),c("time", "Inc", "prevB", "prevA", "prevAB", "prevGC", "ARx", "Doxy1", "Doxy2", "Doxy3"))])) %>% 
  select(RowSum, Doxy1, Doxy2)%>% summarise(min(RowSum), max(RowSum), sum(Doxy1), sum(Doxy2))
doxy_sim50 %>% mutate(RowSum=rowSums(.[setdiff(names(.),c("time", "Inc", "prevB", "prevA", "prevAB", "prevGC", "ARx", "Doxy1", "Doxy2", "Doxy3"))])) %>% 
  select(RowSum, Doxy1, Doxy2)%>% summarise(min(RowSum), max(RowSum), sum(Doxy1), sum(Doxy2))
doxy_sim75 %>% mutate(RowSum=rowSums(.[setdiff(names(.),c("time", "Inc", "prevB", "prevA", "prevAB", "prevGC", "ARx", "Doxy1", "Doxy2", "Doxy3"))])) %>% 
  select(RowSum, Doxy1, Doxy2)%>% summarise(min(RowSum), max(RowSum), sum(Doxy1), sum(Doxy2))
...
```{r}
df_list <- list(doxy_sim0 %>% select(-Inc, -prevB, -prevA, -prevAB, -prevGC, -ARx), 
                doxy_sim10 %>% select(-Inc, -prevB, -prevA, -prevAB, -prevGC, -ARx), 
                doxy_sim25 %>% select(-Inc, -prevB, -prevA, -prevAB, -prevGC, -ARx), 
                doxy_sim50 %>% select(-Inc, -prevB, -prevA, -prevAB, -prevGC, -ARx), 
                doxy_sim75 %>% select(-Inc, -prevB, -prevA, -prevAB, -prevGC, -ARx))
...
doxy_sim_all %>%
  ...
  summarise(CumInc5 = CumInc[time == 1825],
            CumInc10 = CumInc[time == 3650],
            CumInc15 = CumInc[time == 5475],
            CumInc20 = CumInc[time == 7300],
            Time87 = min(time[prevB >= 0.87])/365,
            ARx5 = 1-CumARx[time == 1825]/547056.9,
            ARx20 = 1-CumARx[time == 5475]/2102208)
```
...
#pdf("DOXYPEP_CumInfections.pdf", height = 6, width = 8)
# ggplot() +
#   geom_line(data = doxy_sim_all, aes(x = time/365, y = CumInc, col = factor(DoxyPEP)), size = 1) +
#   geom_point(data = resB, aes(x = MinT/365, y = CumInc, col = factor(DoxyPEP)), size = 3, shape = 19) +
#   theme_classic() + xlab("Years") + ylab("Cumulative Infection Count") + labs(col = "DoxyPEP Uptake") + theme(text = element_text(size=15)) +
...
# #dev.off()
# 
# #pdf("DOXYPEP_CumInfections5.pdf", height = 3, width = 3)
# ggplot() +
#   geom_line(data = doxy_sim_all %>% filter(time <= 1825), aes(x = time/365, y = CumInc, col = factor(DoxyPEP)), size = 1) +
#   geom_point(data = resB %>% filter(MinT <= 1825), aes(x = MinT/365, y = CumInc, col = factor(DoxyPEP)), size = 3, shape = 19) +
#   theme_classic() + xlab("Years") + ylab("") + labs(col = "DoxyPEP Uptake") +
#   scale_color_manual(values = c("#172869", "#1BB6AF", "#A6E000", "#FC6882", "#C70E7B")) +
#   theme(legend.position = "none") + scale_y_continuous(breaks = c(0, 250000, 500000, 750000), labels = c("0", "0.25 M", "0.5 M", "0.75 M"))
  #scale_color_manual(values = c("#172869", "#1BB6AF", "#A6E000", "#FF5300", "#FC6882"))
#dev.off()

#pdf("DOXYPEP_PrevHI.pdf", height = 6, width = 8)
ggplot() +
  ...
  theme_classic() + xlab("Years") + ylab("Gonococcal Infection Prevalence (%)") + labs(col = "DoxyPEP Uptake in High\nActivity Group Only") + theme(text = element_text(size=15), legend.position = c(0.80, 0.25)) +
  scale_color_manual(values = c("#172869", "#1BB6AF", "#A6E000", "#FC6882", "#C70E7B"))
#dev.off()
...
```{r}
#check infection burden by risk group
check <- doxy_sim_long_all %>%
  group_by(time, RiskGroup, InfectState, DoxyPEP) %>%
  filter(InfectState == "Asymptomatic Infected" | InfectState == "Symptomatic Infected") %>%
  summarise(N = sum(individuals))

ggplot(data = check) +
  geom_line(aes(x = time, y = N, col = DoxyPEP)) + 
  facet_grid(InfectState ~ RiskGroup)
```
...
#pdf("DOXYPEP_PRHI.pdf", height = 3, width = 3)
ggplot() +
