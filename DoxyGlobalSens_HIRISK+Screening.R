...
doxyPEP.SI <- function(t, x, parms){
  with(as.list(c(t, x, parms)),{
    ...
    Doxy = prB*(1-kappa)*(1-omega_b)*(gamma*E0 + gamma*Ea)
    ...
    list(der, Inc = Inc, prevA = prevA, prevB = prevB, prevAB = prevAB, prevGC = prevGC, ARx = ARx, BRx = BRx, Doxy = Doxy)
  })
}
...
RunODE <- function (x) {
  ...
  parms$prB <- c(0,0,x) #probability of DoxyPEP set to 0 in sexual activity groups 1-2 (low-intermediate)
  # Screening rate stays the same in sexual activity groups 1-2 (low-intermediate)
  # but increases for high-risk group as a function of doxy-PEP uptake level
  parms$Tm <- c(0.3569368/365, 0.3569368/365, (x*4 + (1-x)*0.3569368)/365)
...
write.csv(res_global, paste0(filepath, "GlobalSensResults_Screen.csv"), row.names=FALSE)
#res_global <- read.csv(paste0(filepath, "GlobalSensResults_Screen.csv"))
