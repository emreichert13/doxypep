...
doxyPEP.SI <- function(t, x, parms){
    ...
    list(der, Inc = Inc, prevA = prevA, prevB = prevB, prevAB = prevAB, prevGC = prevGC, ARx = ARx)
  })
}
```
...
doxy_sim0 %>% mutate(RowSum=rowSums(.[setdiff(names(.),c("time", "Inc", "prevB", "prevA", "prevAB", "prevGC", "ARx"))])) %>% 
  select(RowSum) %>% summarise(min(RowSum), max(RowSum))
doxy_sim10 %>% mutate(RowSum=rowSums(.[setdiff(names(.),c("time", "Inc", "prevB", "prevA", "prevAB", "prevGC", "ARx"))])) %>% 
  select(RowSum)%>% summarise(min(RowSum), max(RowSum))
doxy_sim25 %>% mutate(RowSum=rowSums(.[setdiff(names(.),c("time", "Inc", "prevB", "prevA", "prevAB", "prevGC", "ARx"))])) %>% 
  select(RowSum)%>% summarise(min(RowSum), max(RowSum))
doxy_sim50 %>% mutate(RowSum=rowSums(.[setdiff(names(.),c("time", "Inc", "prevB", "prevA", "prevAB", "prevGC", "ARx"))])) %>% 
  select(RowSum)%>% summarise(min(RowSum), max(RowSum))
doxy_sim75 %>% mutate(RowSum=rowSums(.[setdiff(names(.),c("time", "Inc", "prevB", "prevA", "prevAB", "prevGC", "ARx"))])) %>% 
  select(RowSum)%>% summarise(min(RowSum), max(RowSum))
