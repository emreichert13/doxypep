...
#write.csv(res_global, paste0(filepath, "GlobalSensResults.csv"), row.names=FALSE)
res_global <- read.csv(paste0(filepath, "GlobalSensResults.csv"))
...
res_global %>%
  group_by(DoxyPEP) %>%
  summarise_each(funs(quantile(., 0.25)), TimeA, TimeAB, TimeB, Rel_CI5, Rel_CI20, Rel_A5, Rel_A20, MinPR, Rel_PR5, Rel_PR20) %>%
  ungroup

res_global %>%
  group_by(DoxyPEP) %>%
  summarise_each(funs(quantile(., 0.75)), TimeA, TimeAB, TimeB, Rel_CI5, Rel_CI20, Rel_A5, Rel_A20, MinPR, Rel_PR5, Rel_PR20) %>%
  ungroup
